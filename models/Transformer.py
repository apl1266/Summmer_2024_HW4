...
class TransformerTranslator(nn.Module):
    ...
    def __init__(self, input_size, output_size, device, hidden_dim=128, num_heads=2, dim_feedforward=2048, dim_k=96, dim_v=96, dim_q=96, max_length=43):
        ...
        ##############################################################################
        self.linear_1 = nn.Linear(self.hidden_dim, self.dim_feedforward)
        self.relu = nn.ReLU()
        self.linear_2 = nn.Linear(self.dim_feedforward, self.hidden_dim)
        ...
        ##############################################################################
        self.out_layer = nn.Linear(self.hidden_dim, self.output_size)
        ##############################################################################
        #                               END OF YOUR CODE                             #
        ##############################################################################
    ...
    def forward(self, inputs):
        ...
        #############################################################################
        embeding = self.embed(inputs)
        hiden = self.multi_head_attention(embeding)
        hiden_forward = self.feedforward_layer(hiden)
        outputs = self.final_layer(hiden_forward)
    ...
    def feedforward_layer(self, inputs):
        ...
        outputs = self.norm(torch.add(linear_2, inputs))
