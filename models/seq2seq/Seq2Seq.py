...
class Seq2Seq(nn.Module):
    ...
    def forward(self, source, out_seq_len=None):
        ...
        dec_dim = self.decoder.output_size
        outputs = torch.zeros(batch_size, out_seq_len, dec_dim).to(self.device)
        out, hidden = self.encoder(source)

        outs = source[:, 0].unsqueeze(1)
        for t in range(out_seq_len):
            outt, hidden = self.decoder(outs, hidden, out, self.attention)
            outputs[:, t, :] = outt
            output = outt.argmax(dim=-1).unsqueeze(1)
            outs = output
