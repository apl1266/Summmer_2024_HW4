...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, emb_size, encoder_hidden_size, decoder_hidden_size, dropout=0.2, model_type="RNN"):
        ...
        if self.model_type == "RNN":
            self.model = nn.RNN(self.emb_size, self.encoder_hidden_size, batch_first=True)
        else:
            self.model = nn.LSTM(self.emb_size, self.encoder_hidden_size, batch_first=True)
        self.layer_1 = nn.Linear(self.encoder_hidden_size, self.encoder_hidden_size)
        self.R = nn.ReLU()
        self.layer_2 = nn.Linear(self.encoder_hidden_size, self.decoder_hidden_size)
        self.T = nn.Tanh()
    ...
    def forward(self, input):
        ...
        if self.model_type == "LSTM":
            hidden, c = hidden
        hida = self.layer_1(hidden)
        hida = self.R(hida)
        hida = self.layer_2(hida)
        hidden = self.T(hida)
        if self.model_type == "LSTM":
            hidden = hidden, c
