...
class VanillaRNN(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, output_size):
        ...
        self.layer_1 = nn.Linear(self.input_size + self.hidden_size, self.hidden_size)
        self.tanh = nn.Tanh()

        # initialize block that creates output
        self.layer_2 = nn.Linear(self.input_size + self.hidden_size, self.output_size)
    ...
    def forward(self, input, hidden):
        ...
        inp_h = torch.cat((input, hidden), 1)
        hidden = self.layer_1(inp_h)
        hidden = self.tanh(hidden)
        output = self.layer_2(inp_h)
